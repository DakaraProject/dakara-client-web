...
export default class WorkLinkWidget extends Component {
  ...
  render() {
    const { longLinkType, noEpisodes, noIcon, query, truncatable, workLink } =
      this.props

    // Subtitle if any
    let subtitle
    if (workLink.work.subtitle) {
      subtitle = <span className="subtitle">{workLink.work.subtitle}</span>
    }
    ...
    if (!noIcon && workLink.work.work_type) {
      icon = (
        <span className="work-link-item icon">
          <i className={`las la-${workLink.work.work_type.icon_name}`}></i>
        </span>
      )
    }
        ...
        <span className="title-group">
          <HighlighterQuery
            ...
            searchWords={(q) => {
              ...
              const workTypeQuery =
                q.work_type[workLink.work.work_type.query_name]
              if (workTypeQuery) {
