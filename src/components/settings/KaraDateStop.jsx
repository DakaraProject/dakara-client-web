...
class KaraDateStop extends Component {
  ...
  render() {
    ...
    if (
      this.props.karaokeState.status === Status.pending ||
      this.props.karaokeState.status === null
    )
      return null
    ...
    if (isManager) {
      ...
      karaDateStopWidget = (
        ...
        >
          ...
          <InputField
            id="time_stop"
            defaultValue={
              karaokeDateStop ? dayjs(karaokeDateStop).format('HH:mm') : ''
            }
      ...
      if (karaokeDateStop) {
        karaDateStopWidget = (
          <p>Karaoke stop time: {dayjs(karaokeDateStop).format('HH:mm')}</p>
        )
      } else {
        karaDateStopWidget = <p>Karaoke stop time is not set.</p>
      }
    }
...
KaraDateStop = connect(mapStateToProps, {})(KaraDateStop)
